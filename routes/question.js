...
exports.validate_answer = async function(req, res, next) {
    ...
    var current_date = new Date(moment().tz('Asia/Kolkata').format());

    let question, status, user, scoreAdd, quiz_level;
    try {
        question = await Question.findOne({
            "question_id": question_id
        }, "answer pikacharanswer score quiz_type question_st question_type");
        ...
        if (question.quiz_type == "BONUS") {
            ...
            let UAMObj = new UserAnswerMapping({
                "mht_id": user_mhtid,
                "question_id": question_id,
            ...
            quiz_level = await QuizLevel.findOne({
            ...
            if (isRightAnswer) {
                ...
                await User.updateOne({
                    ...
                    $inc: {
                        "totalscore_month": scoreAddMonth,
                    },
                    $set: {
                        "update_date": current_date,
                        "question_id": question_id
            ...
            //console.log('tet');
            let UAMObj = await UserAnswerMapping.findOne({
                "mht_id": user_mhtid,
                "question_id": question_id
            });
            //console.log(UAMObj);
            if (UAMObj == null) {
                UAMObj = new UserAnswerMapping({
                    "mht_id": user_mhtid,
                    "question_id": question_id,
                    "quiz_type": question.quiz_type,
                    "answer": selected_ans,
...
exports.get_bonus_question = async function(req, res, next) {
    ...
    try {
        ...
        if (question.length > 0) {
            ...
            res.send(200, {
                msg: 'You have already finished Daily Bonus'
            });
        }
        next();
    } catch (error) {
    ...
    try {
        ...
        if (user.totalscore >= app_setting.score_per_lives && user.lives < 3) {
            user = await User.updateOne({
                ...
                $inc: {
                    "lives": 1,
                    "totalscore": (app_setting.score_per_lives * -1)
                }
            });
            let users = await User.findOne({
                "mht_id": mht_id
...
exports.puzzle_completed = async function(req, res, next) {
    ...
    try {
        ...
        await User.updateOne({
            "mht_id": mht_id,
            "$where": "this.lives < " + app_setting.total_lives
        }, {
            $inc: {
                "lives": inc_lives
            }
    ...
    if ((datetimet.getDay()) == 1) {
        ...
        if (user.totalscore_month_update == undefined || user.totalscore_month_update <= datetimet) {
            await User.updateOne({
                ...
                $set: {
                    "totalscore_month": 0,
                    "totalscore_month_update": datetimetMonth
                }
            });
        }
    }
    if (n == 'Monday') {
        ...
        if (user.totalscore_week_update == undefined || user.totalscore_week_update >= datetimet) {
            await User.updateOne({
                ...
                $set: {
                    "totalscore_week": 0,
                    "totalscore_week_update": datetimetWeek
                }
            });
        }
    }
 ...
 */
exports.check_user_score = async function(req, res, next) {
    let mht_id = req.body.mht_id;
    let level = req.body.level;
    let question_st = req.body.question_st;
    try {
        ...
        if (!userScore) {
            ...
            userScore = await UserScore.findOne({
                "mht_id": mht_id,
                "level": level
            });
        }
